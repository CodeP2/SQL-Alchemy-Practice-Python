...
def search_by_name():
    ...
    author = get_valid_imput("What is the name of the author?", author_names_list, "Invalid author!")
    query_by(author, "author")


def search_by_title():
    title_list = entries_by_x.title_list()
    title = get_valid_imput("What is the title?", title_list, "Invalid title!")
    query_by(title, "title")


def search_by_publish_date():
    publish_date_list = entries_by_x.publish_date_list()
    published = get_valid_imput("What is the date of the publication?", publish_date_list, "Invalid publication date!", False, False, True)
    query_by(published, "published")


def search_by_price():
    price_list = entries_by_x.price_list()
    price = get_valid_imput("What is the price of the book?", price_list, "Invalid price!", False, True)
